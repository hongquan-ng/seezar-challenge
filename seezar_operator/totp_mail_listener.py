...
DBOS_CLIENT = DBOSClient(os.getenv("DBOS_DATABASE_URL"))
...
async def listen_mercure(token, account_id):
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        async with aconnect_sse(
            client, method="GET", url=MERCURE_URL, params=params, headers=headers
        ) as event_source:
            event_source.response.raise_for_status()
            print("SSE connection established")
            async for event in event_source.aiter_sse():
                logging.info(f"New event: {event.data}")
...
async def main():
    ...
    await listen_mercure(token, account_id)
